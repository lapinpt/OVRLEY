...
export function headingTapeHasHighlightBar(config) {
  return config.show_indicator && config.indicator_style === 'highlight_bar'
}

export function headingTapeLayout(config) {
  const frameHeight = config.height
  const indicatorSize = config.indicator_size
...
export function headingTickPosition(bodyHeight, config, isMajor) {
  const majorLength = (bodyHeight * config.major_tick_length_pct) / 100
  const minorLength = (bodyHeight * config.minor_tick_length_pct) / 100
...
export function headingLabelBaseline(bodyHeight, config) {
  const majorLength = (bodyHeight * config.major_tick_length_pct) / 100
  return majorLength + config.label_offset + config.label_font_size
}

export function headingLabelBottom(bodyHeight, config) {
  return headingLabelBaseline(bodyHeight, config) + config.label_font_size * HEADING_TAPE_LABEL_DESCENT_PCT
}
...
export function headingTickScaleHeightForRenderedBody(renderedBodyHeight, config) {
  const majorTickRatio = config.major_tick_length_pct / 100
  const fixedLabelStack = config.label_offset + config.label_font_size * (1 + HEADING_TAPE_LABEL_DESCENT_PCT)
 ...
 */
export function visibleTicks(heading, pixelsPerDegree, width, majorTickInterval, minorTicksPerMajor, showMajorTicks, showMinorTicks) {
  if (pixelsPerDegree <= 0 || width <= 0) throw new Error('Heading tape dimensions must be positive')
