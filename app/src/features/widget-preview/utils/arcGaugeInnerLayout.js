...
export function getArcInnerWidgetLayout(data, layout, model) {
  ...
  const unitFontSize = Math.max(model.fontSize * ARC_INNER_WIDGET_UNIT_RATIO, ARC_INNER_WIDGET_MIN_UNIT_FONT_SIZE)
  const valueLineHeight = model.fontSize * ARC_INNER_WIDGET_LINE_HEIGHT
  const unitLineHeight = unitFontSize * ARC_INNER_WIDGET_LINE_HEIGHT
  const gap = Math.max(model.fontSize * 0.08, ARC_INNER_WIDGET_GAP_PX)
