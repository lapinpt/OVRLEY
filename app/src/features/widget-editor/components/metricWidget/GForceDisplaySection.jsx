...
export default function GForceDisplaySection({ widget, updateWidgetData }) {
  ...
  const borderMax = Math.floor((data.diameter - 1) / 2)
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <SliderField
            label="Decimals"
            value={data.label_decimals}
            min={0}
            max={3}
            valueDisplay={String(data.label_decimals)}
            onSliderChange={(label_decimals) => updateGForce({ label_decimals })}
          />
        </div>
        <div className="grid grid-cols-2 gap-4">
