...
struct Sections {
    ...
    /// Free-form exporter details from the `[comments]` section.
    comments: Vec<String>,
...
fn parse_sections(text: &str) -> CoreResult<Sections> {
    ...
    for (line_index, line) in text.lines().enumerate() {
        ...
        match current.as_str() {
            ...
            "comments" => sections.comments.push(trimmed.to_string()),
...
fn build_activity_columns(sections: Sections, file_name: &str) -> CoreResult<ActivityColumns> {
    ...
    let (latitude, longitude) = match coordinate_encoding(&sections) {
        CoordinateEncoding::VboxMinutes => (
            series(layout.latitude, minutes_to_degrees),
            series(layout.longitude, longitude_minutes_to_degrees),
        ),
        CoordinateEncoding::DecimalDegrees => (
            series(layout.latitude, identity),
            series(layout.longitude, identity),
        ),
    };
    ...
    Ok(ActivityColumns {
        ...
        torque: empty(),
        estimated_torque: empty(),
        estimated_power_kw: empty(),
        estimated_power_cv: empty(),
        estimated_gear: empty(),
        stroke_rate: empty(),
        stride_length: empty(),
...
/// Coordinate representation declared by the VBO exporter dialect.
///
/// Racelogic VBOX and RaceChrono through v9 use latitude/longitude minutes,
/// with positive-west longitude. RaceChrono v10 writes signed decimal degrees.
#[derive(Clone, Copy)]
...
/// Selects the coordinate representation without guessing from a geographic value.
///
/// Both minutes and degrees can fall within latitude/longitude ranges, so value-based inference
/// would silently move valid routes. RaceChrono v10 identifies itself in `[comments]`.
fn coordinate_encoding(sections: &Sections) -> CoordinateEncoding {
    let is_racechrono_v10_or_newer = sections.comments.iter().any(|comment| {
        let normalized = comment.trim().to_ascii_lowercase();
        let Some(version) = normalized.strip_prefix("generated by racechrono v") else {
            return false;
        };
        version
